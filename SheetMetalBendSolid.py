...
def WrapBSpline(bspline, Radius, zeroVert, cent, axis, zeroVertNormal):
    ...
    newbspline = bspline
    newbspline.buildFromPolesMultsKnots(
        newpoles,
...
def WrapFace(Face, Radius, axis, normal, zeroVert, cent, zeroVertNormal):
    ...
    for e in Face.Edges:
        ...
        if isinstance(e.Curve, (Part.Circle, Part.ArcOfCircle)):
            ...
            Edges.append(
                WrapBSpline(bspline, Radius, zeroVert, cent, axis, zeroVertNormal)
            )

        elif isinstance(e.Curve, Part.BSplineCurve):
            Edges.append(
                WrapBSpline(e.Curve, Radius, zeroVert, cent, axis, zeroVertNormal)
            )
            ...
            if linenormal.dot(axis) == 0.0 and (dist2 - dist1) == 0.0:
                Point1 = getPointOnCylinder(
                    zeroVert, sp, Radius, cent, axis, zeroVertNormal
                )
                Point2 = getPointOnCylinder(
                    zeroVert, mp, Radius, cent, axis, zeroVertNormal
                )
                Point3 = getPointOnCylinder(
                    zeroVert, ep, Radius, cent, axis, zeroVertNormal
                )
