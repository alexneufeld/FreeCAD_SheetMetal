...
class Simple_node(object):
    ...
    def __init__(
        self, f_idx=None, Parent_node=None, Parent_edge=None, k_factor_lookup=None
    ):
    ...
    def k_Factor(self):
        ...
        if manKFactor is not None:
            ...
            k = get_val_from_range(
                self.k_factor_lookup, self.innerRadius / self.thickness
            )
        if kFactorStandard == "din":
            ...
            raise ValueError(
                "Unrecognized K-factor standard: %s" % (str(kFactorStandard))
            )
        return k
...
class SheetTree(object):
    def __init__(self, TheShape, f_idx, k_factor_lookup):
        ...
        if theVol < 0.0001:
            ...
            if hasattr(self.__Shape.Faces[f_idx], "Surface"):
                ...
                FreeCAD.Console.PrintLog(
                    "lLine number edges: " + str(len(lLine.Edges)) + "\n"
                )
    ...
    def Bend_analysis(self, face_idx, parent_node=None, parent_edge=None):
        ...
        if self.error_code is None:
            ...
            t_node = self.make_new_face_node(
            ...
            for child_info in t_node.child_idx_lists:
                if child_info[0] in self.index_list:
                    FreeCAD.Console.PrintLog(
                        "child in list: " + str(child_info[0]) + "\n"
                    )
...
def getUnfold(k_factor_lookup):
    ...
    if mylist.__len__() == 0:
        ...
        QtGui.QMessageBox.information(
            mw, "Error", """One flat face needs to be selected!"""
        )
    else:
        if mylist.__len__() > 1:
            mw = FreeCADGui.getMainWindow()
            QtGui.QMessageBox.information(
                mw, "Error", """Only one flat face has to be selected!"""
            )
        else:
            ...
            if len(o.SubObjects) > 1:
                mw = FreeCADGui.getMainWindow()
                QtGui.QMessageBox.information(
                    mw, "SubelementError", """Only one flat face has to be selected!"""
                )
            else:
                ...
                if hasattr(subelement, "Surface"):
                    ...
                    if s_type == "<Plane object>":
                        ...
                        TheTree = SheetTree(
                            o.Object.Shape, f_number, k_factor_lookup
                        ...
                        if TheTree.error_code is None:
                            ...
                            FreeCAD.Console.PrintLog(
                                "Analytical time: " + str(endzeit - startzeit) + "\n"
                            )
                            ...
                            if TheTree.error_code is None:
                                ...
                                if TheTree.error_code is None:
                                    ...
                                    FreeCAD.Console.PrintLog(
                                        "time to run the unfold: "
                                        + str(unfoldTime - endzeit)
                                        + "\n"
                                    )
                        ...
                        if TheTree.error_code is not None:
                            if TheTree.error_code == 1:
                                ...
                                ob_Name = ob.Name
                                # print('obn',ob_Name)
                                ob.Label = o.Object.Label + "_copy"
                                FreeCADGui.Selection.clearSelection()
                                FreeCADGui.Selection.addSelection(
                                    ob, o.SubElementNames[0]
                                )
                                ...
                                QtGui.QMessageBox.information(
                                    mw, "Error", unfold_error[TheTree.error_code]
                                )
                        else:
                        ...
                        QtGui.QMessageBox.information(
                            mw,
                            "Selection Error",
                            """Sheet UFO works only with a flat face as starter!\n Select a flat face.""",
                        )
                else:
                    mw = FreeCADGui.getMainWindow()
                    QtGui.QMessageBox.information(
...
def getObjectsByLabelRecursive(doc, label):
    res = _find_objects(doc.Objects, lambda obj: 2 if obj and obj.Label == label else 0)
...
class SMUnfoldTaskPanel:
    ...
    def __init__(self):
        ...
        self.materalDefinitionSheetLayout.setObjectName(
            _fromUtf8("materalDefinitionSheetLayout")
        )
    ...
    def mdsApplyPressed(self):
        SMMessage(
            "INFO: Changed material of '%s' from '%s' to '%s'"
    ...
    def accept(self):
        ...
        global kFactorStandard
        mds_help_url = (
            "https://github.com/shaise/FreeCAD_SheetMetal#material-definition-sheet"
        )
        ...
        if self.checkKfact.isChecked():
            ...
            SMMessage(
                "Manual K-factor is being used: %.2f (%s)"
                % (manKFactor, kFactorStandard)
            )
            ...
            if self.new_mds_name != self.material_sheet_name:
                SMErrorBox(
                    "Please 'Apply' your new Material Definition Sheet name first."
                )
                return
        ...
        if self.material_sheet_name and not self.checkKfact.isChecked():
            ...
            if lookup_sheet is None:
                SMErrorBox(
                    "No Spreadsheet is found containing material definition: %s"
                    % self.material_sheet_name
                )
                return
            ...
            if key_cell is None:
                lookup_sheet_err = (
                    "No cell can be found with name: 'Radius / Thickness'"
                )
            if value_cell is None:
                lookup_sheet_err = (
                    "No cell can be found with name: 'K-factor (ANSI/DIN)'"
                )
            if kFactorStandard is None:
                if options_cell is None:
                    lookup_sheet_err = (
                        "No 'Options' column or 'K-factor (????)' cell found."
                    )

            if lookup_sheet_err is not None:
                lookup_sheet_err += "<p>"
                lookup_sheet_err += (
                    "Check your Material Definition Sheet's contents.<br />"
                )
                lookup_sheet_err += (
                    "Refer to <a href='%s'>SheetMetal/README</a> if you are unsure about how to continue."
                    % mds_help_url
                )
                lookup_sheet_err += "</p>"
                ...
                while True:
                    ...
                    if option == "K-factor standard":
                        if kFactorStandard is not None:
                            SMErrorBox(
                                "Multiple K-factor definitions in %s",
                                self.material_sheet_name,
                            )
                            return
            ...
            if kFactorStandard not in ["ansi", "din"]:
                SMErrorBox(
                    "Invalid K-factor standard: %s \nin %s"
                    % (kFactorStandard, self.material_sheet_name)
                )
                return

            if kFactorStandard is None:
                SMErrorBox(
                    "'K-factor standard' option is required (ANSI or DIN) in %s"
                    % self.material_sheet_name
                )
                return
            ...
            msg += (
                "<li>Or use a <a href='%s'>Material Definition Sheet</a></li>"
                % mds_help_url
            )
            msg += "</ol>"
            SMErrorBox(msg)
        ...
        try:
            ...
            if err_cd == 1:
                try:
                    genstep = True
                    s, foldComp, norm, thename, err_cd, fSel, obN = getUnfold(
                        k_factor_lookup
                    )
                    ...
                    QtGui.QMessageBox.question(
                        None, "Warning", msg, QtGui.QMessageBox.Ok
                    )
            else:
                s = None
                QtGui.QApplication.restoreOverrideCursor()
            ...
            if genSketchChecked:
                ...
                if bendSketchChecked:
                    ...
                    try:
                        ...
                        # Part.show(newface)
                        self.generateSketch(
                            owEdgs, "Unfold_Sketch_Outline", self.genColor.colorF()
                        )
                ...
                if len(foldLines) > 0 and bendSketchChecked:
                    ...
                    self.generateSketch(
                        foldEdges, "Unfold_Sketch_bends", self.bendColor.colorF()
                    )
    ...
    def retranslateUi(self):
        self.form.setWindowTitle(
            _translate("SheetMetal", "Unfold sheet metal object", None)
        )
        self.checkSketch.setText(
            _translate("SheetMetal", "Generate projection sketch", None)
        )
        self.checkSeparate.setText(
            _translate("SheetMetal", "Separate projection layers", None)
        )
        ...
        self.InternalLbl.setText(
            _translate("SheetMetal", "    Internal lines color", None)
        )
        self.kfactorAnsi.setText(_translate("SheetMetal", "ANSI", None))
        self.kfactorDin.setText(_translate("SheetMetal", "DIN", None))
        self.checkUseMds.setText(
            _translate("SMUnfoldTaskPanel", "Use Material Definition Sheet", None)
        )
